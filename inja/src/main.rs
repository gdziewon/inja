...
use windows::Win32::System::Threading::{WaitForSingleObject, CreateRemoteThread, OpenProcess, PROCESS_VM_OPERATION, PROCESS_VM_WRITE, PROCESS_CREATE_THREAD, };
use windows::Win32::System::Memory::{VirtualAllocEx, VirtualFreeEx, MEM_RESERVE, MEM_COMMIT, PAGE_READWRITE, MEM_RELEASE};
...
fn inject(dll_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    let pid = get_process_id(PROCESS_NAME)?;

    let dll_path_obj = Path::new(dll_path).canonicalize()?;
    if !dll_path_obj.exists() {
        return Err("dll doesnt exist".into());
    }
    let dll_path = dll_path_obj.to_str().unwrap();
...
fn main() {
    inject(r".\target\debug\injectant.dll").unwrap();
}
